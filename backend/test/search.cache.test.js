...
await test('POST /api/search serves repeated requests from cache and exposes alternatives', async (t) => {
  ...
  const payload = { query: 'learn guitar', maxResults: 2, pageToken: 'abc', tone: 'coach', expertiseLevel: 'beginner' };
  ...
  assert.equal(firstJson.learningPreferences.tone, 'coach');
  assert.equal(firstJson.learningPreferences.expertiseLevel, 'beginner');
  assert.ok(Array.isArray(firstJson.learningPath.checkpoints));
  assert.ok(Array.isArray(firstJson.nextActions));
  assert.ok(firstJson.queryAnalysis && typeof firstJson.queryAnalysis.complexity === 'string');
  assert.ok(Array.isArray(firstJson.suggestions));
