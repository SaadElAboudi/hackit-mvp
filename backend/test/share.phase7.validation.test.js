...
await test('POST /api/rooms/:id/share invalid target returns BAD_REQUEST envelope', async (t) => {
    forceMongoReady();
    ...
    const res = await requestJson({
        ...
        body: { target: 'jira' },
        headers: { 'x-user-id': 'user_phase7_1' },
    });
    ...
    assert.match(String(json.message || ''), /target/i);
    assert.ok(typeof json.requestId === 'string' && json.requestId.length > 0);
});

await test('GET /api/rooms/:id/share/history invalid status returns BAD_REQUEST envelope', async (t) => {
    forceMongoReady();
    t.after(() => restoreMongoReady());
    ...
    const res = await requestJson({
        port,
        method: 'GET',
        path: '/api/rooms/507f191e810c19729de860ea/share/history?status=done',
        headers: { 'x-user-id': 'user_phase7_2' },
    });
